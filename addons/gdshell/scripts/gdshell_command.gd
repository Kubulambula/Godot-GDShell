...
class CommandResult:
	...
	func _to_string() -> String:
		return "{GDShellCommand.CommandResult: err: %s, err_string: %s, data: %s}" % [err, err_string, str(data)]
...
#var _PARENT_SESSION: GDShellSession
...
func execute(_command: String) -> CommandResult:
	return null
	#return await _PARENT_SESSION._handle_execute(command)
