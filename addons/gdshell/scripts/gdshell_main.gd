...
func _ready() -> void:
	var x: GDShellCommandParser.ParserResult = GDShellCommandParser.parse("echo | echo", null)
	print("Input: ", x.input)
	print("Status: ", x.status)
	print("AST: ", x.ast)
	print("Tokens: ", x.tokens)
	print("Err index: ", x.err_token_index)
	print("Err: ", x.err_string)
...
#	print(x.result)
#	var a = ""
#	for i in x.err_index:
#		a += " "
#	print(a+"^")
#	print(GDShellCommandParser._tokenize_text("a 'abc'", 0).consumed)
#	print(x.status)
